        ...
        protected virtual void CombineScripts(HttpContext context, bool isSaaS, string scriptName, string culture, string version)
        {
            ...
            if (isSaaS)
            {
                if (ApplicationServices.IsTouchClient)
                    ...
                    sb.AppendFormat(string.Format("$('<link></link>').appendTo($('head')).attr({{ href: '{0}/App_Themes/IProc/_Theme_Aquarium.css?{0}', type: 'text/css', rel: 'stylesheet' }});", ApplicationServices.Version), baseUrl);
                try
                {
                ...
                sb.AppendFormat("var __targetFramework='4.8';__tf=4.0;__cothost='appfactory';__appInfo='IPROC|{0}';", BusinessRules.JavaScriptString(context.User.Identity.Name));
        ...
        protected virtual void BeginResponsePadding(HttpRequest request, StreamWriter sw)
        {
            ...
            else
            {
                if ((request.HttpMethod == "GET") && UriRestConfig.IsJSONPRequest(request))
                {
                    ...
                    sw.Write("IProc=typeof IProc=='undefined'?{{}}:IProc;IProc.{0}=", instance);
                }
            }
        }
    ...
    public partial class MSGraphOAuthHandlerBase : OAuthHandler
    {
        ...
        public override void SetUserAvatar(MembershipUser user)
        {
            if (ApplicationServices.IsSiteContentEnabled)
                try
                {
                    var request = WebRequest.Create("https://graph.microsoft.com/v2.0/me/photo/$value");
