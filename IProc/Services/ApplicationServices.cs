...
namespace IProc.Services
{
        ...
        public static string ValidationKey
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(key) || key.Contains("AutoGenerate"))
                    key = "36088AFF97D9A0B315F050F726ECA5F71AA98C856BFD96D2CDDC5CFF8A7A3ACDCC6A43F32F2D2734952BC5C51E2B7D15A271050C22542BBE4D750BA64517228B";
                return key;
            }
        }
    ...
    public partial class ApplicationServicesBase
    {
        ...
        public static bool IsTouchClient
        {
            get
            {
                var isMobile = HttpContext.Current.Items["ApplicationServices_IsTouchClient"];
                if (isMobile == null)
                {
        ...
        {
            IProc.Services.ApplicationServices.Initialize();
        }
        ...
        public static void RegisterStandardMembershipAccounts()
        {
            if (AuthorizationIsSupported)
            {
                ...
                if (Membership.GetUser("admin") == null)
                {
                    ...
                    admin = Membership.CreateUser("admin", "admin123%", "admin@IProc.com", "ASP.NET", "Code OnTime", true, out status);
                    user = Membership.CreateUser("user", "user123%", "user@IProc.com", "ASP.NET", "Code OnTime", true, out status);
        ...
        public static bool ClientIsUsingTouchUI()
        {
            if (!EnableMobileClient)
                return false;
            var request = HttpContext.Current.Request;
            var mobileCookie = request.Cookies["appfactorytouchui"];
        ...
        {
            foreach (var c in p.Header.Controls)
                if (c is HtmlLink)
                {
                    var l = ((HtmlLink)(c));
                    if (l.ID == "IProcTheme")
    ...
    public class ApplicationSessionStateBase : SessionStateStoreProviderBase
    {
        ...
        public override void SetAndReleaseItemExclusive(HttpContext context, string id, SessionStateStoreData item, object lockId, bool newItem)
        {
            ...
            if (newItem)
            {
                ...
                using (var cmd = CreateSqlText("INSERT INTO aspnet_Sessions (SessionId, ApplicationName, Created, Expires, LockDate, LockId, Timeout, Locked, SessionItems, Flags) Values(@SessionId, @ApplicationName, @Created, @Expires, @LockDate, @LockId , @Timeout, @Locked, @SessionItems, @Flags)"))
                {
            ...
            {
                using (var cmd = CreateSqlText("UPDATE aspnet_Sessions SET Expires = @Expires, SessionItems = @SessionItems, Locked = @Locked WHERE SessionId = @SessionId AND ApplicationName = @ApplicationName AND LockId = @LockId"))
                {
                    cmd.AddParameter("SessionId", id);
                    cmd.AddParameter("Expires", DateTime.Now.AddMinutes(((double)(item.Timeout))));
                    cmd.AddParameter("SessionItems", sessItems);
        ...
        private SessionStateStoreData GetSessionStoreItem(bool lockRecord, HttpContext context, string id, out bool locked, out TimeSpan lockAge, out object lockId, out SessionStateActions actionFlags)
        {
            ...
            try
            {
                ...
                if (lockRecord)
                {
                    using (var cmd = CreateSqlText("UPDATE aspnet_Sessions SET Locked = @LockedTrue, LockDate = @LockDate WHERE SessionId = @SessionId AND ApplicationName = @ApplicationName AND Locked = @LockedFalse AND Expires > @Expires"))
                    {
                ...
                using (var cmd = CreateSqlText("SELECT Expires, SessionItems, LockId, LockDate, Flags, Timeout FROM aspnet_Sessions WHERE SessionId = @SessionId AND ApplicationName = @ApplicationName"))
                {
                    cmd.AddParameter("SessionId", id);
                ...
                if (deleteData)
                {
                    using (var cmd = CreateSqlText("DELETE FROM aspnet_Sessions WHERE SessionId = @SessionId AND ApplicationName = @ApplicationName"))
                    {
                        cmd.AddParameter("SessionId", id);
                        cmd.ExecuteNonQuery();
                    }
                ...
                {
                    lockId = (((int)(lockId)) + 1);
                    using (var cmd = CreateSqlText("UPDATE aspnet_Sessions SET LockId = @LockId, Flags = 0 WHERE SessionId = @SessionId AND ApplicationName = @ApplicationName"))
                    {
                        cmd.AddParameter("lockId", lockId);
                        cmd.AddParameter("SessionId", id);
                        cmd.ExecuteNonQuery();
        ...
        public SqlText CreateSqlText(string sql)
        {
            ...
            cmd.Name = "IProc Session State Provider";
        ...
        {
            using (var cmd = CreateSqlText("UPDATE aspnet_Sessions SET Locked = 0, Expires = @Expires WHERE SessionId = @SessionId AND ApplicationName = @ApplicationName AND LockId = @LockId"))
            {
        ...
        {
            using (var cmd = CreateSqlText("DELETE FROM aspnet_Sessions WHERE SessionId = @SessionId AND ApplicationName = @ApplicationName AND LockId = @LockId"))
            {
                cmd.AddParameter("SessionId", id);
                cmd.AddParameter("LockId", lockId);
                cmd.ExecuteNonQuery();
        ...
        {
            using (var cmd = CreateSqlText("INSERT INTO aspnet_Sessions (SessionId, ApplicationName, Created, Expires, LockDate, LockId, Timeout, Locked, Flags) Values(@SessionId, @ApplicationName, @Created, @Expires, @LockDate, @LockId, @Timeout, @Locked, @Flags)"))
            {
                cmd.AddParameter("SessionId", id);
                cmd.AddParameter("Created", DateTime.Now);
        ...
        {
            using (var cmd = CreateSqlText("UPDATE aspnet_Sessions SET Expires = @Expires WHERE SessionId = @SessionId AND ApplicationName = @ApplicationName"))
            {
                cmd.AddParameter("Expires", DateTime.Now.AddMinutes(_config.Timeout.TotalMinutes));
                cmd.AddParameter("SessionId", id);
                cmd.ExecuteNonQuery();
