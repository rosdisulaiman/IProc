...
namespace IProc.Data
{
    public class ControllerConfiguration
    {
        ...
        protected virtual void Initialize(XPathNavigator navigator)
        {
            ...
            {
                var t = ApplicationServices.StringToType("IProc.Rules.SharedBusinessRules");
                if (t != null)
...
<action id=""a5"" commandName=""Edit"" causesValidation=""true"" />
        ...
        public string LoadLayout(string view)
        {
            ...
            while (tryLoad)
            {
                ...
                {
                    var stream = GetType().Assembly.GetManifestResourceStream(string.Format("IProc.Views.{0}.{1}.html", this.ControllerName, view));
                    if (stream != null)
                    {
    ...
    public class ControllerConfigurationUtility
    {
        ...
        public static string[] GetDataControllerList()
        {
            ...
            {
                var m = Regex.Match(r.Value, "^IProc\\.(controllers\\.)?(.+?)\\.xml$", RegexOptions.IgnoreCase);
                if (m.Success)
                {
